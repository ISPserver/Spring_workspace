...
public class ExcelManager {
	...
	public ExcelManager() {
		...
		try {
			...
			//파일접근은 성공한 상태이므로, 이 시점부터는 쉬트(Sheet)에 접근한다.
			XSSFSheet sheet =workbook.getSheetAt(0); //첫번째 쉬트 선택
			
			//이 시점부턴 sheet를 제어할 수 있으므로, 총 몇건이 존재하는지부터 알아본다.
			int rows = sheet.getPhysicalNumberOfRows();//데이터가 채워진 Row 수
			System.out.println("레코드 수는 : "+rows);
			
			//로우와 컬럼 수만큼 반복문 실행하며, 데이터를 제어해보자
			for(int i=0; i<rows; i++) {
				XSSFRow row = sheet.getRow(i);//하나의 Row 접근
				//컬럼 수 만큼 반복
				int columCount = row.getPhysicalNumberOfCells();
				for(int a=0; a<columCount; a++) {
					XSSFCell cell = row.getCell(a);//컬럼 한개 접근
					//엑셀의 자료형을 판단해 그에 맞게 접근
					if(cell.getCellType()==CellType.STRING) {//Cell의 자료형이 String인 경우
